...
# Controlla che i comandi siano disponibili
command -v hisat2 || { echo "HISAT2 not found"; exit 1; }
...
# Costruisci l'indice (solo se non esiste già)
if [ ! -f "${INDEX_DIR}/niphotrichum_chr3.1.ht2" ]; then
    echo "### Building HISAT2 index from chr3 soft-masked assembly"
    hisat2-build -p ${THREADS} ${ASSEMBLY} ${INDEX_DIR}/niphotrichum_chr3
fi

# Definisci i campioni con percorsi assoluti
SAMPLES=(
...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    # Converti in BAM e ordina
    samtools sort -@ ${THREADS} -o ${BAM_DIR}/${NAME}.sorted.bam ${BAM_DIR}/${NAME}.sam
    rm ${BAM_DIR}/${NAME}.sam  # Rimuovi il file temporaneo
